//! N Parameter Sweep - EXP-4-003
//! Test quality vs Gaussian count to establish rate-distortion curve
...
fn main() {
    ...
    println!("EXP-4-003: N Parameter Sweep");
    println!("Image: {}×{}", target.width, target.height);
    println!();

    // Test grid sizes: 10×10 to 50×50
    let grid_sizes = vec![10, 15, 20, 25, 30, 35, 40, 45, 50];

    println!("Testing quality vs N with 50 iterations");
    println!();
    println!("Grid | N     | PSNR    | dB/N");
    println!("-----|-------|---------|--------");
    ...
    for grid_size in grid_sizes {
        ...
        let db_per_n = psnr / n as f32;

        println!("{:4} | {:5} | {:7.2} | {:.5}", grid_size, n, psnr, db_per_n);
    }
}
