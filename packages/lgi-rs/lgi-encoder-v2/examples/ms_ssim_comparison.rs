...
fn main() {
    ...
    println!("MS-SSIM Loss Comparison\n");

    let encoder = EncoderV2::new(target.clone()).unwrap();
    let gaussians_init = encoder.initialize_gaussians_guided(20);

    // Test 1: L2 Loss (baseline)
    println!("═══ L2 Loss (Baseline) ═══");
    let mut gaussians_l2 = gaussians_init.clone();
    let mut optimizer_l2 = OptimizerV2::new_with_gpu();
    ...
    let msssim_l2 = lgi_core::ms_ssim::MSSSIM::default().compute(&target, &rendered_l2);

    println!("  PSNR:   {:.2} dB", psnr_l2);
    println!("  MS-SSIM: {:.4}\n", msssim_l2);

    // Test 2: MS-SSIM Loss
    println!("═══ MS-SSIM Loss ═══");
    let mut gaussians_msssim = gaussians_init.clone();
    let mut optimizer_msssim = OptimizerV2::new_with_gpu_and_ms_ssim();
    optimizer_msssim.max_iterations = 50;
    optimizer_msssim.optimize(&mut gaussians_msssim, &target);

    let rendered_msssim = RendererV2::render(&gaussians_msssim, target.width, target.height);
    let psnr_msssim = compute_psnr(&rendered_msssim, &target);
    let msssim_msssim = lgi_core::ms_ssim::MSSSIM::default().compute(&target, &rendered_msssim);

    println!("  PSNR:   {:.2} dB", psnr_msssim);
    println!("  MS-SSIM: {:.4}\n", msssim_msssim);

    // Comparison
    println!("═══ Comparison ═══");
    println!("             | PSNR     | MS-SSIM");
    println!("-------------|----------|--------");
    println!("L2 Loss      | {:.2} dB | {:.4}", psnr_l2, msssim_l2);
    println!("MS-SSIM Loss | {:.2} dB | {:.4}", psnr_msssim, msssim_msssim);
    println!("Delta        | {:+.2} dB | {:+.4}", psnr_msssim - psnr_l2, msssim_msssim - msssim_l2);

    if msssim_msssim > msssim_l2 {
        println!("\n✅ MS-SSIM loss improves perceptual quality!");
    }
}
