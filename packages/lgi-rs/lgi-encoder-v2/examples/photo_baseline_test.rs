//! Photo Baseline Test - Quick photo quality check with conditional anisotropy
...
fn main() {
    ...
    println!("Photo: {}×{}", target.width, target.height);

    let encoder = EncoderV2::new(target.clone()).unwrap();
    let mut gaussians = encoder.initialize_gaussians_guided(30);  // 30×30 = 900

    println!("Gaussians: {}", gaussians.len());
    println!("Optimizing (50 iters)...");
