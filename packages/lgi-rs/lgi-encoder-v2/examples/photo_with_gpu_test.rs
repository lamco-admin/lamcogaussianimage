...
fn main() {
    ...
    println!("Photo: {}×{}, N=400, 30 iters", target.width, target.height);

    let encoder = EncoderV2::new(target.clone()).unwrap();
    let mut gaussians = encoder.initialize_gaussians_guided(20);

    let mut optimizer = OptimizerV2::new_with_gpu();  // GPU ENABLED
    optimizer.max_iterations = 30;

    println!("Has GPU: {}", optimizer.has_gpu());
    println!("Optimizing...");
