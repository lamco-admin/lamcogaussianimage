//! Edge-Weighted Loss Test - EXP-4-007
//! Test if edge-weighted gradients improve edge quality
...
fn main() {
    ...
    println!("EXP-4-007: Edge-Weighted Loss Test");
    println!("Image: {}×{}", target.width, target.height);
    println!();

    // Test 1: Standard L2 (baseline)
    println!("Test 1: Standard L2");
    let encoder1 = EncoderV2::new(target.clone()).unwrap();
    let mut gaussians_l2 = encoder1.initialize_gaussians_guided(30);

    let mut optimizer_l2 = OptimizerV2::default();
    optimizer_l2.max_iterations = 50;
    optimizer_l2.use_edge_weighted = false;
    ...
    println!("Standard L2 PSNR: {:.2} dB\n", psnr_l2);

    // Test 2: Edge-weighted L2
    println!("Test 2: Edge-Weighted L2");
    let encoder2 = EncoderV2::new(target.clone()).unwrap();
    let mut gaussians_ew = encoder2.initialize_gaussians_guided(30);

    let mut optimizer_ew = OptimizerV2::default();
    optimizer_ew.max_iterations = 50;
    optimizer_ew.use_edge_weighted = true;  // Enable edge weighting

    optimizer_ew.optimize(&mut gaussians_ew, &target);

    let rendered_ew = RendererV2::render(&gaussians_ew, target.width, target.height);
    let psnr_ew = compute_psnr(&rendered_ew, &target);

    println!("Edge-Weighted PSNR: {:.2} dB\n", psnr_ew);

    // Summary
    println!("===== SUMMARY =====");
    println!("Standard L2:    {:.2} dB", psnr_l2);
    println!("Edge-Weighted:  {:.2} dB", psnr_ew);
    println!("Delta: {:+.2} dB", psnr_ew - psnr_l2);

    if psnr_ew > psnr_l2 + 0.5 {
        println!("✅ Edge-weighted loss improves quality by {:.2} dB", psnr_ew - psnr_l2);
    } else if psnr_ew > psnr_l2 {
        println!("⚠️ Edge-weighted loss helps slightly (+{:.2} dB)", psnr_ew - psnr_l2);
    } else {
        println!("❌ Edge-weighted loss doesn't improve overall PSNR");
    }
}
